...
class Preprocessing:
    def __init__(self, place, is_test=False):
        ...
        """
        self.connections = None  # Dictionary to store how streets connected to each other
        self.place = place
        self.roundabout = None  # To store elements of roundabout type
        ...
        self.df_pro = None  # The dataset to store the data
    ...
    def first_filtering(self):
        ...
        self.df_pro = to_remove
        self.create_connections_dict()
        self.df_pro = self.df_pro.to_crs(project_crs).dropna(subset=['name'])
        self.df_pro = self.df_pro[self.df_pro['name'] != '']
        ...
        self.df_pro['name'] = self.df_pro['name'].apply(convert_to_list)
        self.df_pro['highway'] = self.df_pro['highway'].apply(convert_to_list)

        self.df_pro['angle'] = self.df_pro['bearing'].apply(lambda x: x if x < 180 else x - 180)
        self.df_pro['length'] = self.df_pro.length
    ...
    def create_connections_dict(self):
        ...
        df_analysis = self.df_pro.copy()

        # Step 2: Perform a spatial join
        # This operation joins the street data with itself to find connections
        gpd.sjoin(df_analysis, self.df_pro)
        s_join_analysis = gpd.sjoin(df_analysis, self.df_pro)
        ...
        if self.is_junction:
            roundabout = self.df_pro[self.df_pro['junction'].isin(['roundabout', 'circular'])]
            self.df_pro = self.df_pro[
                ~((self.df_pro['junction'] == 'roundabout') | (self.df_pro['junction'] == 'circular'))]
            ...
            s_join_roundabout = gpd.sjoin(self.df_pro, roundabout_geo)
            ...
            self.roundabout = roundabout_geo
            self.connections = connections2
